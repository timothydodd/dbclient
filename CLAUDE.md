...
- **SqlPackageService.cs** — DACPAC export/import by shelling out to the SqlPackage CLI (not bundled; located via PATH, `~/.dotnet/tools`, and the SQL Server `DAC\bin` folders, cached after first lookup). Database-node context menu on SQL Server connections shows `Export DACPAC...` / `Import DACPAC...` when found, else a disabled hint item. `DacpacExportDialog` (schema-only vs. data), `DacpacImportDialog` (file, target database name, block-on-data-loss, script-only), and `SqlPackageDialog` (streams process output, Cancel kills the process). Connection strings come from `SqlServerConnection.GetExternalConnectionStringAsync` (brings up the SSH tunnel first); they're redacted in the log but visible in the OS process list while SqlPackage runs.
